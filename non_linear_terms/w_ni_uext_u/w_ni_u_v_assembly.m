function [ res ] = w_ni_u_v_assembly( params, paramsP, grid, qdeg )
...
for i = 1:1:length(tria_index)
    res_internal(gids(tria_index(i),:),gids(tria_index(i),:)) =...
        res_internal(gids(tria_index(i),:),gids(tria_index(i),:)) + w_ni_u_v_integral...
        ( params, paramsP, grid, tria_index(i), local_vertex_index(i), qdeg );
end
...
for i = 1:1:length(tria_index)
    ...
    if tria_index_neighbour > 0
        res_external(gids(tria_index(i),:),gids(tria_index_neighbour,:)) = ...
            res_external(gids(tria_index(i),:),gids(tria_index_neighbour,:)) + ...
            w_ni_uext_v_integral( params, paramsP, grid, tria_index(i),...

function [ res ] = n_u_tensor_jump_n_phi_tensor_jump_minus_minus_local( llcoord,...
...
%     llcoord), params);
% velocity_basis_self_theory = sum(velocity_basis_self,2);
...
if tria_index_neighbour >0
    local_vertex_index_neighbour = find(grid.NBI(tria_index_neighbour,:)...
        == tria_index);
...
for a = 1:1:params.dimrange
    for i = a:params.dimrange:params.ndofs_per_element  % 2 is for 2-d grid
        for j = a:params.dimrange:params.ndofs_per_element
            res(i,j) = res(i,j) + ...
                (N_neighbour * velocity_basis_neighbour(i))*...
                (N_neighbour * velocity_basis_neighbour(j))';
        end
    end
end
else 
    disp('Dirichlet bundary for negative part. returning zeros');
end
end   
% check and understand formula for n \otimes u and n \otimes \phi
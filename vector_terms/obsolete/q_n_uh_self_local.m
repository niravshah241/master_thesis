function [ res ] = q_n_uh_self_local( llcoord, grid, params, tria_index,...
    local_vertex_index)
%Q_N_UH_SELF_LOCAL Summary of this function goes here
...
velocity_basis_self = ldg_evaluate_basis(llocal2local(grid,...
face_index_self,llcoord),params);

N = [grid.NX(tria_index,local_vertex_index) grid.NY(tria_index,local_vertex_index)];
jump_velocity_plus = velocity_basis_self*N';

for i=1:1:paramsP.ndofs_per_element
    for j=1:1:params.ndofs_per_element    
        res(i,j)=pressure_basis_average(i,:)*jump_velocity_plus(j,:)';
    end
end
end